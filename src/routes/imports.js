...
async function getOrgContext(req) {
  ...
  if (!orgId) {
    return { ok: false, status: 400, message: "Missing org context (x-org-id)." };
  }
...
function detectRowType(row) {
  const hasDealSignals =
    ...
    getValue(row, ["amount", "value", "deal value", "expected revenue", "revenue"]);
    ...
    for (const row of rows) {
      ...
      if (rowType === "deal") {
        const clientName = String(
          getValue(row, ["client", "client name", "account", "company", "country"]) || ""
        ).trim();
        ...
        const normalizedProbability =
          rawProbability > 1
            ? Math.min(rawProbability / 100, 1)
            : Math.max(rawProbability, 0);
        ...
        await dealsCollection.insertOne({
          ...
          amount: safeNum(
            getValue(row, [
              ...
              "expected revenue",
              "revenue",
              "annual local currency country manager cost",
            ]),
            0
          ),
